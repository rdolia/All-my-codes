#check ggplot2 layers.. 7 layers
#data, asthetics, geometrics - which kind of graph, bar, scatter etc...,facets- ?,statistics - moving average
#or median etc... , coordinates - , theme - colours etc..
#for univariate analysis - use histogram like a bar plot
#bivariate analysis - height and weight - give actual data through a scatter plot. when both x and y are
#numerical values.
#stacked bar chart is a bar chart with stacked values subgroup of sales like unblled amount and subgroups of which project
#line graphs used for time series analysis or trends. like stocks
#box plot - used to measure spread of the data. 
#doughnut chart
#funnel chart - check example from google
...
gg<-gg + theme(axis.text.x=element_text(angle=50, size=10, vjust=0.2))
gg
...
ggplot(diamonds, aes(price, carat, color=price))+geom_point()+
...
histog <- ggplot(Cars93,aes(RPM))+
  geom_bar(width = 50.5,fill="firebrick",color="cadetblue2")+
  ggtitle("Histogram")
histog
